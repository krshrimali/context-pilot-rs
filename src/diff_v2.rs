...
#[derive(PartialEq, Eq, Debug)]
pub enum DiffCases {
    FewLinesReplacedWithSingleLine,
    FewLinesReplacedWithFewLines,
    SingleLineDeleted,
    FewLinesDeleted,
    SingleLineReplacedWithAnotherSingleLine,
...
pub fn reorder_map(
    ...
    match category {
        Some(DiffCases::FewLinesReplacedWithSingleLine) => {
            ...
            // Update or remove replaced lines
            for l_no in s_line_no..e_line_no {
                if !replaced_content_line_numbers.contains(&l_no) {
                    map.remove(&l_no);
                } else {
                    let new_content = line_change_after
                        ...
                        .to_string();
                    if let Some(line_details) = map.get_mut(&l_no) {
                        // FIX: Update all entries, not just [0]
                        for detail in line_details.iter_mut() {
                            detail.commit_hashes.push(commit_hash.clone());
                            detail.content = new_content.clone();
                        }
                    } else {
                        map.insert(
                            l_no,
            ...
            // Shift lines after the replaced block
            let mut to_remove_map: HashMap<u32, Vec<LineDetail>> = HashMap::new();
            let keys: Vec<u32> = map.keys().cloned().collect();
            for l_no in keys.iter() {
                if *l_no >= e_line_no {
                    let new_idx = *l_no - (line_change_before.change_count - 1);
                    if let Some(data) = map.get(l_no) {
                        to_remove_map.insert(new_idx, data.clone());
                    }
                }
            }
            // Remove old positions before reinserting
            for key in to_remove_map.keys() {
                map.remove(key);
            }
            for (l_no, line_detail) in to_remove_map {
                map.insert(l_no, line_detail);
            }
            // Remove trailing lines that are now out of range
            let max_line = *map.keys().max().unwrap_or(&0);
            for key in map.keys().cloned().collect::<Vec<u32>>() {
                if key > max_line.saturating_sub(line_change_before.change_count - 1) {
                    map.remove(&key);
                }
            }
        }
        Some(DiffCases::FewLinesReplacedWithFewLines) => {
            ...
            if diff > 0 {
                ...
                for l_no in keys.iter() {
                    if *l_no >= line_change_after.start_line_number + line_change_after.change_count
                    {
                        let new_idx: i32 = *l_no as i32 + diff;
                        if let Some(data) = map.remove(l_no) {
                            to_remove_map.insert(new_idx as u32, data);
                        }
                    }
                }
                for (l_no, line_detail) in to_remove_map {
        ...
        Some(DiffCases::SingleLineDeleted) => {
            ...
            for l_no in keys.iter() {
                if *l_no > s_line_no {
                    let new_idx = *l_no - 1;
                    if let Some(data) = map.remove(l_no) {
                        to_remove_map.insert(new_idx, data);
                    }
                }
            }
            for (l_no, line_detail) in to_remove_map {
        ...
        Some(DiffCases::SingleLineReplacedWithAnotherSingleLine) => {
            let s_line_no = line_change_after.start_line_number;
            if let Some(line_detail) = map.get_mut(&s_line_no) {
                let new_content = line_change_after
                    .changed_content
                    .get((s_line_no - line_change_after.start_line_number) as usize)
                    .unwrap()
