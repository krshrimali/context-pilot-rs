...
async fn test_index_file_mode_does_not_affect_workspace_indexing() {
    ...
    if !result1.is_empty() {
        ...
        println!("No results to store for file 1 in test environment");
    }
    ...
    if !result2.is_empty() {
        ...
        println!("No results to store for file 2 in test environment");
    }

    // Only proceed with checking if both files have results
    if !result1.is_empty() && !result2.is_empty() {
        ...
        println!("Skipping final checks as one or both files have no results in test environment");
    }
}
